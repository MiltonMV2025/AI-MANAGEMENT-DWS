...
type QaRunStatus = "pending" | "completed" | "partial" | "error";
type TrelloSyncStatus = "pending" | "sent" | "failed";

type QaRunRow = {
  id: string;
  project_id: string;
  source_type: "qa_input";
  status: QaRunStatus;
...
function toApiUseCase(useCase: QaUseCaseRow) {
  return {
    id: useCase.id,
    title: useCase.title,
    objective: useCase.objective,
    preconditions: useCase.preconditions,
    steps: parseSteps(useCase.test_steps_json),
    expectedResult: useCase.expected_result,
    priority: useCase.priority,
    testType: useCase.test_type,
    trello: {
      status: useCase.trello_status,
      cardId: useCase.trello_card_id,
      cardUrl: useCase.trello_card_url,
      error: useCase.trello_error,
    },
...
async function getRunUseCases(runId: string): Promise<QaUseCaseRow[]> {
  const result = await query<QaUseCaseRow>(
    `
    SELECT
      id, run_id, title, objective, preconditions, test_steps_json, expected_result,
      priority, test_type, trello_card_id, trello_card_url, trello_status, trello_error, created_at
    FROM qa_use_cases
    WHERE run_id = $1
...
async function syncUseCasesToTrello(useCases: QaUseCaseRow[]): Promise<SyncSummary> {
  ...
  for (const useCase of useCases) {
    try {
      ...
      await query(
        ...
        WHERE id = $1
        `,
        [useCase.id, created.cardId, created.cardUrl]
      );
      ...
      failed.push({ useCaseId: useCase.id, reason });
    }
  }
...
async function processQaRun(params: {
  projectId: string;
  input: string;
  userId: string;
}): Promise<{ run: QaRunRow; useCases: QaUseCaseRow[]; trelloSync: SyncSummary }> {
  await assertProjectOwnership(params.projectId, params.userId);

  const insertedRun = await query<QaRunRow>(
    `
    INSERT INTO qa_generation_runs (project_id, created_by_user_id, source_type, raw_input, status)
    OUTPUT inserted.id, inserted.project_id, inserted.source_type, inserted.status, inserted.ai_model, inserted.error_message, inserted.created_at
    VALUES ($1, $2, 'qa_input', $3, 'pending')
    `,
    [params.projectId, params.userId, params.input]
  );

  const run = insertedRun.rows[0];

  try {
    const aiResult = await extractQaUseCasesFromInput(params.input);

    const insertedUseCases = await runInTransaction(async (txQuery) => {
      await txQuery(
        `
        ...
        WHERE id = $1
        `,
        [run.id, aiResult.model, JSON.stringify(aiResult.rawResponse)]
      );

      const useCases: QaUseCaseRow[] = [];
      for (const useCase of aiResult.useCases) {
        const inserted = await txQuery<QaUseCaseRow>(
          `
          INSERT INTO qa_use_cases
            (run_id, title, objective, preconditions, test_steps_json, expected_result, priority, test_type, trello_status)
          OUTPUT
            inserted.id, inserted.run_id, inserted.title, inserted.objective, inserted.preconditions, inserted.test_steps_json,
            inserted.expected_result, inserted.priority, inserted.test_type, inserted.trello_card_id, inserted.trello_card_url,
        ...
        useCases.push(inserted.rows[0]);
      }

      return useCases;
    });

    const trelloSync = await syncUseCasesToTrello(insertedUseCases);
    const runStatus: QaRunStatus = trelloSync.failedCount > 0 ? "partial" : "completed";
    ...
    const refreshedUseCases = await getRunUseCases(run.id);
    return {
      run: {
        ...run,
        status: runStatus,
      ...
      useCases: refreshedUseCases,
      trelloSync,
    };
  } catch (error) {
    const message = error instanceof Error ? error.message : "QA generation failed";
    await query(
      `
      UPDATE qa_generation_runs
      SET status = 'error',
...
projectRouter.post("/projects/:projectId/qa-runs", async (req, res) => {
  if (!isTrelloConfigured()) {
    res.status(400).json({ message: "Trello QA list variables are not fully configured." });
    return;
  }
  ...
  try {
    const result = await processQaRun({
      projectId,
      input,
    ...
    res.status(201).json({
      run: {
        ...
        sourceType: result.run.source_type,
        status: result.run.status,
        aiModel: result.run.ai_model,
        errorMessage: result.run.error_message,
        createdAt: result.run.created_at,
      },
      qaUseCases: result.useCases.map((useCase) => toApiUseCase(useCase)),
      trelloSync: result.trelloSync,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "QA processing error";
    res.status(502).json({ message });
  }
});

projectRouter.get("/projects/:projectId/qa-runs", async (req, res) => {
  const { projectId } = req.params;
  try {
    await assertProjectOwnership(projectId, req.authUser!.userId);
  } catch {
    res.status(404).json({ message: "Project not found or access denied." });
    return;
  }

  const runs = await query<QaRunRow>(
    `
    SELECT id, project_id, source_type, status, ai_model, error_message, created_at
    FROM qa_generation_runs
    WHERE project_id = $1
    ORDER BY created_at DESC
    `,
    [projectId]
...
projectRouter.get("/qa-runs/:runId/review", async (req, res) => {
  const { runId } = req.params;
  const userId = req.authUser!.userId;

  const runResult = await query<QaRunRow>(
    `
    SELECT TOP 1 r.id, r.project_id, r.source_type, r.status, r.ai_model, r.error_message, r.created_at
    FROM qa_generation_runs r
  ...
  const useCases = await getRunUseCases(runId);
  ...
  res.json({
    ...
    qaUseCases: useCases.map((useCase) => toApiUseCase(useCase)),
  });
});

projectRouter.post("/qa-runs/:runId/retry-trello", async (req, res) => {
  if (!isTrelloConfigured()) {
    res.status(400).json({ message: "Trello QA list variables are not fully configured." });
    return;
  }
  ...
  const pendingOrFailed = await query<QaUseCaseRow>(
    `
    SELECT
      id, run_id, title, objective, preconditions, test_steps_json, expected_result,
      priority, test_type, trello_card_id, trello_card_url, trello_status, trello_error, created_at
    FROM qa_use_cases
    WHERE run_id = $1 AND trello_status <> 'sent'
    ORDER BY created_at ASC
    `,
    [runId]
  );

  const trelloSync = await syncUseCasesToTrello(pendingOrFailed.rows);
  ...
  const missingCount = Number(remaining.rows[0]?.missing_count ?? 0);
  const runStatus: QaRunStatus = missingCount > 0 ? "partial" : "completed";
  ...
  const refreshedUseCases = await getRunUseCases(runId);
  ...
  res.json({
    ...
    qaUseCases: refreshedUseCases.map((useCase) => toApiUseCase(useCase)),
    trelloSync,
  });
});
